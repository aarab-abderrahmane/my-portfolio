...
export const ChatOverlay: React.FC<ChatOverlayProps> = ({ isActive, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Background Dimmer */}
      <div 
        className="fixed inset-0 z-[99] bg-black/20 backdrop-blur-[2px] transition-all"
        onClick={onClose}
      />
      ...
      <div className="fixed bottom-0 left-0 right-0 z-[101] pointer-events-none flex flex-col items-center justify-end pb-8 px-4 h-full">
        ...
        <div className={`w-full flex flex-col gap-4 pointer-events-auto transition-all duration-500 ease-in-out ${isExpanded ? 'max-w-4xl' : 'max-w-2xl'}`}>
          ...
          {messages.length > 0 && (
            <div className={`w-full bg-[#111111]/80 backdrop-blur-3xl border border-white/10 rounded-[32px] p-4 md:p-6 shadow-2xl flex flex-col gap-4 animate-in slide-in-from-bottom-10 fade-in duration-500 overflow-hidden transition-all duration-500 ${isExpanded ? 'h-[75vh]' : ''}`}>
          ...
          >
            ...
            <div className="relative flex items-center bg-[#1a1a1a]/80 backdrop-blur-3xl border border-white/10 rounded-full p-2 shadow-2xl">
              <input
                type="text"
                value={inputValue}
