...
export function GenerationProvider({ children }: { children: ReactNode }) {
  ...
  const startGeneration = useCallback((projectId: string, onComplete?: () => void) => {
    setStates((prev) => {
      ...
      intervalsRef.current[projectId] = setInterval(() => {
        ...
        if (elapsed >= GENERATION_DURATION) {
          ...
          setStates((prev) => ({
            ...prev,
            [projectId]: {
              progress: 100, sparklesFading: true, showImages: true,
              showGeneratingFooter: false, generationComplete: true, remainingEta: "",
            },
          }))
          const callback = callbacksRef.current[projectId]
          if (callback) { callback(); delete callbacksRef.current[projectId] }
          return
        }

        setStates((prev) => ({
          ...prev,
          [projectId]: { ...prev[projectId], progress: Math.round(newProgress), remainingEta: `~${remaining}s` },
        }))
      }, 50)
      ...
      return {
        ...prev,
        [projectId]: {
          progress: 0, sparklesFading: false, showImages: false,
          generationComplete: false, showGeneratingFooter: true, remainingEta: "~10s",
        },
      }
    })
  }, [])
  ...
  useEffect(() => {
    return () => { Object.values(intervalsRef.current).forEach((interval) => clearInterval(interval)) }
  }, [])
